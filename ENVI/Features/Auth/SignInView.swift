    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: platform.iconName)
                ...
                .background(ENVITheme.surfaceLow(for: .dark))
                .clipShape(Circle())
        }
        .buttonStyle(.plain)
...
private struct LinkOnlyPlatformSheet: View {
    let platform: SocialPlatform
    @Environment(\.dismiss) private var dismiss

    var body: some View {
        VStack(spacing: ENVISpacing.lg) {
            Text("Link \(platform.rawValue) In Profile")
                .font(.spaceMonoBold(14))
                .foregroundColor(.white)

            Text("For now, \(platform.rawValue) is available as a connected publishing account, not as a sign-in provider.")
                .font(.interRegular(13))
